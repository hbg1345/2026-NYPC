"""
Local judge/simulator to run two copies of main.exe (the same C++ agent)
against each other, following GameRules.md.
...
This is not the official contest judge binary, but it implements section 5's
map generation (symmetric point sampling, the auxiliary A=24-point boundary
ring, the double Voronoi-centroid relaxation, and ridge-based adjacency) and
the section 1-4 game engine (build/move/train/combat/income/upkeep) as
faithfully to the written rules as practical. One necessary deviation: the
final region centers are rounded to the nearest integer, since the protocol
(and main.c++'s std::stoll parsing) transmits x/y as integers -- the rules
...
HQ_LEVELS = {
    ...
    3: dict(cost=1200, whp=6, hp=20, turret=2, train_cap=2, work_cap=3),
    4: dict(cost=2400, whp=7, hp=25, turret=3, train_cap=2, work_cap=4),
    5: dict(cost=3600, whp=8, hp=30, turret=3, train_cap=3, work_cap=5),
}
BASE_LEVELS = {
    1: dict(cost=300, hp=6, turret=1, work_cap=1),
    2: dict(cost=600, hp=12, turret=1, work_cap=2),
    3: dict(cost=1000, hp=18, turret=2, work_cap=3),
}
...
def gen_map(rng, force_np=None, force_kp=None):
    ...
    Np = force_np if force_np is not None else rng.randint(25, 54)
    N = 2 * Np + 1
    kp_lo = math.ceil((0.15 * N - 1) / 2)
    kp_hi = math.floor((0.2 * N - 1) / 2)
    Kp = force_kp if force_kp is not None else rng.randint(kp_lo, kp_hi)
...
class Engine:
    ...
    # -------------------- MOVE --------------------
    def apply_move_orders(self, side, orders):
    ...
    def resolve_combat(self):
        damage_log = {}   # (side,num) -> total dmg this day
        siege_log = {}    # region -> total dmg this day
        ...
        for r in range(self.N):
            ...
            for w, d in zip(aW, a_dmg):
                if d > 0:
                    w.hp -= d
                    damage_log[(w.side, w.num)] = damage_log.get((w.side, w.num), 0) + d
            for w, d in zip(bW, b_dmg):
                if d > 0:
                    w.hp -= d
                    damage_log[(w.side, w.num)] = damage_log.get((w.side, w.num), 0) + d
            ...
            if b is not None:
                ...
                if lost > 0:
                    siege_log[r] = siege_log.get(r, 0) + lost
                    b.hp -= lost
        ...
        return damage_log, siege_log, dead_regions
...
def send_turn_result(agent, self_side, my_cd, opp_cd, upgrades_applied, trained, moved, damage_log, siege_log):
    a = agent
    a.send("TURN")
    a.send(f"COUNTDOWN {self_side} {my_cd} {OTHER[self_side]} {opp_cd}")
    ...
    a.send(f"DAMAGE {len(damage_log)}")
    for (side, num), dmg in damage_log.items():
        a.send(f"- {side}{num} {dmg}")
    a.send(f"SIEGE {len(siege_log)}")
    for region, dmg in siege_log.items():
        a.send(f"- {region} {dmg}")
    a.send("END")
...
def resolve_runnable(path):
    ...
    if need_build:
        ...
        r = subprocess.run(["g++", "-O2", "-std=c++17", "-o", exe_path, path], capture_output=True, text=True)
        if r.returncode != 0:
...
def run_one_game(exe_a, exe_b, seed, record=True, map_override=None,
    ...
    for turn in range(MAX_TURN):
        agentA.send(f"START DAY {turn}")
        agentB.send(f"START DAY {turn}")
        ...
        # ---- MOVE ----
        eng.apply_move_orders("A", movesA)
        eng.apply_move_orders("B", movesB)
        ...
        # ---- COMBAT ----
        damage_log, siege_log, dead_regions = eng.resolve_combat()
        ...
        send_turn_result(agentA, "A", 999, 999, upgrades_applied, trained, moved, damage_log, siege_log)
        send_turn_result(agentB, "B", 999, 999, upgrades_applied, trained, moved, damage_log, siege_log)
    ...
    if winner is None and reason is None:
        ...
        if hqa > hqb:
            winner, reason = "A", f"200 days elapsed, HQ hp {hqa} vs {hqb}"
        elif hqb > hqa:
            winner, reason = "B", f"200 days elapsed, HQ hp {hqa} vs {hqb}"
        else:
            winner, reason = None, f"200 days elapsed, HQ hp tied at {hqa} -> draw"
...
def main():
    ap = argparse.ArgumentParser(description="Simulate two agents (.cpp source or .exe) against each other.")
    ...
    exe_a = resolve_runnable(args.agent1)
    exe_b = resolve_runnable(args.agent2) if args.agent2 else None
