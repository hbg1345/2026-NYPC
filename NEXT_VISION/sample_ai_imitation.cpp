  // A separate strategy model reconstructed from sample_ai_replay/1 (32).txt.
  //
  // This submission source is fully standalone: the protocol/state parser and
  // combat helpers are embedded below, followed by the independent imitation
  // strategy state machine.  No other project source file is required.
  #define NEXT_VISION_NO_MAIN
  #include <algorithm>
  #include <cassert>
  #include <cmath>
  #include <cstdlib>
  ...
  struct ImitationStrategyState {
    bool initialized = false;
    std::vector<int> home_targets;
    std::vector<int> core_targets;
    std::vector<int> last_attack_turn;
    std::vector<std::pair<WarriorId, int>> scout_started;
    int corridor_probe_start_turn = -1;
    std::vector<int> corridor_probe_targets;
    std::vector<std::pair<WarriorId, int>> corridor_scout_targets;
    int hq_probe_start_turn = -1;
    bool hq_assault_started = false;
  };

  static bool contains_region(const std::vector<int> &regions, int region) {
    return std::find(regions.begin(), regions.end(), region) != regions.end();
  }

  static Actions decide_sample_imitation(const GameState &S, const GameMap &M,
                                        const Paths &P,
                                        ImitationStrategyState &strategy,
                                        int turn) {
    constexpr int INF = 1000000000;
    constexpr int ATTACK_MARGIN = 3;
    constexpr int SCOUT_LIFETIME = 22;
    constexpr int HQ_PROBE_FORCE_THRESHOLD = 100;

    Actions a;
    int budget = S.gold;
    int forced_builder_train_n = 0;
    int hq_probe_move_reserve = 0;
    const int N = M.N;
    const Side enemy_side = opposite(M.my_side);
    const int center = center_stronghold(M);
    ...
    std::vector<char> is_strong(N, false), is_visible(N, false);
    for (int region : M.strongholds)
      is_strong[region] = true;
    for (int region : S.visible)
      is_visible[region] = true;
    ...
    if (!strategy.initialized) {
      ...
      strategy.last_attack_turn.assign(N, -1000);

      for (int region : M.strongholds) {
        if (P.dist[M.my_hq][region] <= P.dist[M.opp_hq][region])
          strategy.home_targets.push_back(region);
      }

      // The replay leaves exactly three home-side strongholds for its late
      // three-builder expansion.  Everything else forms the opening core: the
      // center plus the strongholds that connect the HQ to it with the fewest
      // combined route hops.  This yields six core bases on replay 1 (32) and
      // four on replay 1 (34), rather than overfitting every map to six.
      const int core_base_limit =
          strategy.home_targets.empty()
              ? 0
              : std::max(1, (int)strategy.home_targets.size() - 3);
      if (center >= 0 && contains_region(strategy.home_targets, center))
        strategy.core_targets.push_back(center);
      std::vector<int> connectors;
      for (int region : strategy.home_targets)
        if (region != center)
          connectors.push_back(region);
      std::sort(connectors.begin(), connectors.end(), [&](int lhs, int rhs) {
        const int lhs_score = P.hops[M.my_hq][lhs] + P.hops[lhs][center];
        const int rhs_score = P.hops[M.my_hq][rhs] + P.hops[rhs][center];
        if (lhs_score != rhs_score)
          return lhs_score < rhs_score;
        if (P.hops[M.my_hq][lhs] != P.hops[M.my_hq][rhs])
          return P.hops[M.my_hq][lhs] < P.hops[M.my_hq][rhs];
        return lhs < rhs;
      });
      for (int region : connectors) {
        if ((int)strategy.core_targets.size() >= core_base_limit)
          break;
        strategy.core_targets.push_back(region);
      }
    }

    std::vector<int> my_at(N, 0), enemy_at(N, 0), stationary_regular_at(N, 0);
    int my_alive = 0;
    for (const auto &w : S.warriors) {
      if (w.id.side == M.my_side) {
        ++my_alive;
        ++my_at[w.region];
        if (w.state == WState::STATIONARY && !is_scout_purpose(w.purpose))
          ++stationary_regular_at[w.region];
      } else {
        ++enemy_at[w.region];
      }
    }

    if (my_alive >= HQ_PROBE_FORCE_THRESHOLD &&
        strategy.hq_probe_start_turn < 0) {
      strategy.hq_probe_start_turn = turn;
      dbg::note(turn, "HQ_PROBE start army=" + std::to_string(my_alive));
    }

    const Building *hq = find_building(S, M.my_hq);
    const int hq_level = hq != nullptr ? hq->level : 1;
    // Once HQ technology is complete, production becomes the permanent gold
    // sink.  Preserve a full turn of training before any paid movement, base
    // upgrade, or repair can consume the same money.
    const int max_hq_training_reserve =
        hq_level >= HQ_MAX_LEVEL
            ? std::min(S.gold,
                       TRAIN_COST * HQ_LEVELS[HQ_MAX_LEVEL].train_cap)
            : 0;
    // A threatened workplace keeps its economic workers plus enough bodies to
    // meet the currently visible local wave.  These reservations are shared by
    // construction, scouting, rallying, and attack selection.
    std::vector<int> keep_at(N, 0);
    for (const auto &b : S.buildings)
      if (b.side == M.my_side)
        keep_at[b.region] = b.work_cap();
    for (const auto &b : S.buildings) {
      if (b.side != M.my_side)
        continue;
      int local_threat = 0;
      for (const auto &w : S.warriors)
        if (w.id.side == enemy_side &&
            P.vision_hops[w.region][b.region] <= HOP_VISION)
          ++local_threat;
      if (local_threat > 0)
        keep_at[b.region] = std::max(keep_at[b.region], local_threat + 1);
    }

    std::vector<WarriorId> assigned;
    std::vector<int> leaving(N, 0), incoming(N, 0);
    auto used = [&](WarriorId id) {
      return std::find(assigned.begin(), assigned.end(), id) != assigned.end();
    };
    auto target_will_be_ours = [&](int region) {
      const Building *b = building_at[region];
      if (b != nullptr && b->side == M.my_side)
        return true;
      return std::find(a.upgrades.begin(), a.upgrades.end(), region) !=
            a.upgrades.end();
    };
    auto can_leave = [&](const Warrior &w) {
      if (is_scout_purpose(w.purpose))
        return true;
      const Building *b = building_at[w.region];
      if (b == nullptr || b->side != M.my_side)
        return true;
      return stationary_regular_at[w.region] - leaving[w.region] >
            keep_at[w.region];
    };
    auto queue_move = [&](const Warrior &w, int target, WPurpose purpose,
                          bool preserve_work = true,
                          bool preserve_training = true) {
      if (w.id.side != M.my_side || w.state != WState::STATIONARY || used(w.id) ||
          target < 0 || target >= N || target == w.region ||
          (preserve_work && !can_leave(w)))
        return false;
      const int cost = target_will_be_ours(target) ? 0 : MOVE_COST;
      if (budget < cost)
        return false;
      if (cost > 0 && preserve_training &&
          budget - cost <
              std::max(max_hq_training_reserve, hq_probe_move_reserve))
        return false;
      budget -= cost;
      assigned.push_back(w.id);
      if (!is_scout_purpose(w.purpose))
    ...
    // A post-threshold HQ snapshot has priority over development and training.
    // Pool the stationary force across the map, retaining only one last body at
    // each own building.  Once the HQ is known to be capturable this is an
    // all-in finish, so filled work slots no longer outrank the winning attack.
    const Building *early_enemy_hq = building_at[M.opp_hq];
    const bool early_fresh_hq_snapshot =
        strategy.hq_probe_start_turn >= 0 && is_visible[M.opp_hq] &&
        early_enemy_hq != nullptr && early_enemy_hq->side == enemy_side &&
        early_enemy_hq->type == BType::HQ &&
        early_enemy_hq->last_seen_turn == S.turn &&
        early_enemy_hq->last_seen_turn >= strategy.hq_probe_start_turn &&
        S.enemy_region_count_turn[M.opp_hq] == S.turn;
    if (!strategy.hq_assault_started && early_fresh_hq_snapshot) {
      std::vector<int> hq_defenders;
      for (const auto &intel : S.enemy_memory)
        if (intel.region == M.opp_hq &&
            (!is_visible[M.opp_hq] || intel.visible))
          hq_defenders.push_back(std::max(0, intel.hp));

      struct HqRushCandidate {
        const Warrior *warrior;
        int travel;
      };
      std::vector<HqRushCandidate> candidates;
      for (int source = 0; source < N; ++source) {
        if (source == M.opp_hq)
          continue;
        std::vector<const Warrior *> source_group;
        for (const auto &w : S.warriors) {
          if (w.id.side != M.my_side || w.region != source ||
              w.state != WState::STATIONARY ||
              is_scout_purpose(w.purpose))
            continue;
          if (w.purpose == WPurpose::BUILD && w.target >= 0 && w.target < N &&
              building_at[w.target] == nullptr)
            continue;
          source_group.push_back(&w);
        }
        std::sort(source_group.begin(), source_group.end(),
                  [](const Warrior *lhs, const Warrior *rhs) {
                    if (lhs->hp != rhs->hp)
                      return lhs->hp > rhs->hp;
                    return lhs->id.num < rhs->id.num;
                  });
        const Building *source_building = building_at[source];
        const int last_guard =
            source_building != nullptr && source_building->side == M.my_side
                ? 1
                : 0;
        const int movable =
            std::max(0, stationary_regular_at[source] - last_guard);
        if ((int)source_group.size() > movable)
          source_group.resize(movable);
        const int travel = P.hops[source][M.opp_hq];
        if (travel >= INF)
          continue;
        for (const Warrior *w : source_group)
          candidates.push_back({w, travel});
      }
      std::sort(candidates.begin(), candidates.end(),
                [](const HqRushCandidate &lhs,
                   const HqRushCandidate &rhs) {
                  if (lhs.warrior->hp != rhs.warrior->hp)
                    return lhs.warrior->hp > rhs.warrior->hp;
                  if (lhs.travel != rhs.travel)
                    return lhs.travel < rhs.travel;
                  return lhs.warrior->id.num < rhs.warrior->id.num;
                });

      std::vector<int> available_hps;
      available_hps.reserve(candidates.size());
      for (const auto &candidate : candidates)
        available_hps.push_back(candidate.warrior->hp);

      // Moving troops cannot be retargeted this turn, but their strength tells
      // us whether saving the MOVE gold is worthwhile.  Stop max-HQ training
      // until the currently stationary part of that winning force is ready.
      std::vector<int> total_committable_hps;
      for (const auto &w : S.warriors) {
        if (w.id.side != M.my_side || is_scout_purpose(w.purpose))
          continue;
        if (w.purpose == WPurpose::BUILD && w.target >= 0 && w.target < N &&
            building_at[w.target] == nullptr)
          continue;
        total_committable_hps.push_back(std::max(0, w.hp));
      }
      const int total_simulated_need = minimum_attackers(
          *early_enemy_hq, hq_defenders, total_committable_hps, 30);
      if (total_simulated_need < INF)
        hq_probe_move_reserve =
            (total_simulated_need + ATTACK_MARGIN) * MOVE_COST;

      const int simulated_need =
          minimum_attackers(*early_enemy_hq, hq_defenders, available_hps, 30);
      const int attack_count =
          simulated_need >= INF ? INF : simulated_need + ATTACK_MARGIN;
      int farthest_travel = 0;
      if (attack_count < INF && (int)candidates.size() >= attack_count)
        for (int i = 0; i < attack_count; ++i)
          farthest_travel = std::max(farthest_travel, candidates[i].travel);
      const bool can_launch =
          attack_count < INF && (int)candidates.size() >= attack_count &&
          budget >= attack_count * MOVE_COST;

      if (can_launch) {
        int sent = 0;
        for (int i = 0; i < attack_count; ++i)
          if (queue_move(*candidates[i].warrior, M.opp_hq,
                         WPurpose::ATTACK, false, false))
            ++sent;
        if (sent == attack_count) {
          strategy.hq_assault_started = true;
          hq_probe_move_reserve = 0;
          dbg::note(turn, "HQ_DIRECT_ASSAULT defenders=" +
                              std::to_string(hq_defenders.size()) + " sent=" +
                              std::to_string(sent) + " farthest=" +
                              std::to_string(farthest_travel));
        }
      } else {
        dbg::note(turn, "HQ_PROBE snapshot defenders=" +
                            std::to_string(hq_defenders.size()) +
                            " available=" +
                            std::to_string(candidates.size()) + " need=" +
                            std::to_string(attack_count) + " gold=" +
                            std::to_string(budget) + " move_reserve=" +
                            std::to_string(hq_probe_move_reserve) +
                            " direct=0");
      }
    }

    // Dispatch and advance the threshold HQ scout before any economy, rally,
    // or ordinary scouting pass can consume it.  HQ intelligence is urgent at
    // this force size, so follow the stable weighted route instead of
    // recomputing a safety detour that can oscillate as visibility changes.
    const bool early_hq_probe_active = strategy.hq_probe_start_turn >= 0 &&
                                       !strategy.hq_assault_started;
    if (early_hq_probe_active) {
      auto hq_scout_leg = [&](const Warrior &scout) {
        if (P.vision_hops[scout.region][M.opp_hq] <= HOP_VISION)
          return -1;
        const int next = P.nxt[scout.region][M.opp_hq];
        return next >= 0 && next != scout.region && next != M.opp_hq
                   ? next
                   : -1;
      };

      const Warrior *hq_scout = nullptr;
      for (const auto &w : S.warriors)
        if (w.id.side == M.my_side && w.purpose == WPurpose::HQ_SCOUT &&
            (hq_scout == nullptr || w.id.num < hq_scout->id.num))
          hq_scout = &w;

      if (hq_scout != nullptr && hq_scout->state == WState::STATIONARY &&
          P.vision_hops[hq_scout->region][M.opp_hq] > HOP_VISION) {
        const int leg = hq_scout_leg(*hq_scout);
        if (leg >= 0 && queue_move(*hq_scout, leg, WPurpose::HQ_SCOUT,
                                   false, false))
          dbg::note(turn, "HQ_PROBE advance " +
                              format_warrior(hq_scout->id) + " R" +
                              std::to_string(hq_scout->region) + "->R" +
                              std::to_string(leg));
      } else if (hq_scout == nullptr) {
        const Warrior *best = nullptr;
        int best_leg = -1;
        int best_surplus = -1;
        int best_hops = INF;
        for (const auto &w : S.warriors) {
          ...
          if (w.purpose == WPurpose::BUILD && w.target >= 0 && w.target < N &&
              building_at[w.target] == nullptr)
            continue;
          const int leg = hq_scout_leg(w);
          if (leg < 0)
            continue;
          const int surplus = can_leave(w) ? 1 : 0;
          const int hops = P.vision_hops[w.region][M.opp_hq];
          if (best == nullptr || surplus > best_surplus ||
              (surplus == best_surplus && hops < best_hops) ||
              (surplus == best_surplus && hops == best_hops &&
               w.id.num < best->id.num)) {
            best = &w;
            best_leg = leg;
        ...
        if (best != nullptr &&
            queue_move(*best, best_leg, WPurpose::HQ_SCOUT, false, false))
          dbg::note(turn, "HQ_PROBE assign " + format_warrior(best->id) +
                              " leg=R" + std::to_string(best_leg) +
                              " hops=" + std::to_string(best_hops));
      }
    }

    auto queued_upgrade = [&](int region) {
      return std::find(a.upgrades.begin(), a.upgrades.end(), region) !=
            a.upgrades.end();
    };
    auto queue_upgrade = [&](int region) {
      if (region < 0 || region >= N || queued_upgrade(region) ||
          my_at[region] <= 0 || enemy_at[region] > 0)
        return false;
      const Building *b = building_at[region];
    ...
    // The opening HQ level is the replay's only unconditional technology buy:
    // purchase level two at exact affordability.  Later HQ levels are unlocked
    // by the observed development stages below, not by a gold multiplier.
    if (hq != nullptr && hq->level == 1 && queue_upgrade(M.my_hq))
      dbg::note(turn, "HQ_STAGE opening level=1->2 gold=" +
                          std::to_string(S.gold));

    // Construction at an already reached stronghold is the hard top priority.
    // Frontmost sites are completed first if several waves arrive together.
    std::vector<int> arrived_builds;
    for (int region : M.strongholds)
      if (building_at[region] == nullptr && my_at[region] > 0 &&
          enemy_at[region] == 0)
        arrived_builds.push_back(region);
    std::sort(arrived_builds.begin(), arrived_builds.end(), [&](int lhs, int rhs) {
      if (P.dist[M.opp_hq][lhs] != P.dist[M.opp_hq][rhs])
        return P.dist[M.opp_hq][lhs] < P.dist[M.opp_hq][rhs];
      return lhs < rhs;
    });
    for (int region : arrived_builds)
      queue_upgrade(region);

    auto own_base = [&](int region) {
      const Building *b = building_at[region];
      return b != nullptr && b->side == M.my_side && b->type == BType::BASE;
    };
    auto has_builder_for = [&](int target) {
      for (const auto &w : S.warriors)
        if (w.id.side == M.my_side && w.purpose == WPurpose::BUILD &&
            w.target == target && building_at[target] == nullptr)
          return true;
      for (const auto &move : a.moves)
        if (move.purpose == WPurpose::BUILD && move.target == target)
          return true;
      return false;
    };
    auto count_pending_builds = [&]() {
      int count = 0;
      for (int region : M.strongholds)
        if (building_at[region] == nullptr && has_builder_for(region))
          ++count;
      return count;
    };
    auto core_owned_count = [&]() {
      int count = 0;
      for (int region : strategy.core_targets)
        if (own_base(region) ||
            (building_at[region] == nullptr && queued_upgrade(region)))
          ++count;
      return count;
    };

    // Dispatch one frontier builder at a time early.  After a base is complete,
    // a surplus worker standing there is preferred over an HQ worker, producing
    // the same chained expansion seen in B3: HQ->186->170 and
    // B2: 158->133->131.
    auto dispatch_builder = [&](const std::vector<int> &targets,
                                int pending_limit, bool prefer_center) {
      if (count_pending_builds() >= pending_limit)
        return false;
      const Warrior *best_warrior = nullptr;
      int best_target = -1;
      int best_source_is_front = -1;
      int best_hops = INF;

      int owned_core = core_owned_count();
      for (int target : targets) {
        if (own_base(target) || building_at[target] != nullptr ||
            has_builder_for(target) || enemy_at[target] > 0)
          continue;
        // The first two bases grow from the HQ.  The center is then forced before
        // the remaining connector strongholds.
        long long phase_priority = 0;
        if (prefer_center && target == center && owned_core >= 2)
          phase_priority = 3;
        else if (owned_core < 2)
          phase_priority = -(long long)std::llround(P.dist[M.my_hq][target]);
        else
          phase_priority = 1;

        for (const auto &w : S.warriors) {
          const bool can_replace_hq_worker =
              w.region == M.my_hq && hq != nullptr &&
              forced_builder_train_n < HQ_LEVELS[hq_level].train_cap &&
              budget >= MOVE_COST + TRAIN_COST;
          if (w.id.side != M.my_side || w.state != WState::STATIONARY ||
              used(w.id) || is_scout_purpose(w.purpose) ||
              (!can_leave(w) && !can_replace_hq_worker))
            continue;
          if (w.purpose == WPurpose::BUILD &&
              building_at[w.target] == nullptr)
            continue;
          ...
          if (hops >= INF)
            continue;
          const Building *source = building_at[w.region];
          const int source_is_front =
              source != nullptr && source->side == M.my_side &&
                      source->type == BType::BASE
                  ? 1
                  : 0;
          const auto score = std::make_tuple(phase_priority, source_is_front,
                                            -hops, -w.id.num);
          const auto best_score = std::make_tuple(
              best_target == -1 ? -(long long)INF * INF :
                  ((prefer_center && best_target == center && owned_core >= 2)
                      ? 3LL
                      : (owned_core < 2
                            ? -(long long)std::llround(
                                  P.dist[M.my_hq][best_target])
                            : 1LL)),
              best_source_is_front, -best_hops,
              best_warrior == nullptr ? -INF : -best_warrior->id.num);
          if (best_warrior == nullptr || score > best_score) {
            best_warrior = &w;
            best_target = target;
            best_source_is_front = source_is_front;
            best_hops = hops;
          }
        }
      }
      if (best_warrior == nullptr)
        return false;
      const bool forced_center =
          prefer_center && best_target == center && owned_core >= 2;
      // Ordinary HQ builders wait until the construction fund is within reach;
      // the center-race builder is the exception and departs as soon as MOVE +
      // one replacement TRAIN are affordable.
      if (best_warrior->region == M.my_hq && !forced_center && budget < 340)
        return false;
      if (can_leave(*best_warrior))
        return queue_move(*best_warrior, best_target, WPurpose::BUILD);
      if (budget < MOVE_COST + TRAIN_COST ||
          !queue_move(*best_warrior, best_target, WPurpose::BUILD, false))
        return false;
      budget -= TRAIN_COST;
      ++forced_builder_train_n;
      return true;
    };

    const int raw_core_owned = core_owned_count();
    const bool core_deadlock_fallback =
        turn >= 120 && raw_core_owned + 1 >= (int)strategy.core_targets.size();
    bool all_core_owned = true;
    for (int region : strategy.core_targets)
      if (!own_base(region))
        all_core_owned = false;
    all_core_owned = all_core_owned || core_deadlock_fallback;
    if (!all_core_owned && hq_level >= 2)
      dispatch_builder(strategy.core_targets, 1, true);

    // HQ level three follows completion of the map-sized opening core.  The
    // purchase fires at exact affordability; base depth starts only after this
    // level has actually resolved.
    if (hq != nullptr && hq->level == 2 && all_core_owned &&
        queue_upgrade(M.my_hq))
      dbg::note(turn, "HQ_STAGE core_complete level=2->3 gold=" +
                          std::to_string(S.gold) + " core=" +
                          std::to_string(strategy.core_targets.size()));

    // A visible enemy on a mandatory/core site does not make the target
    // disappear.  Feed the contested site with every economic surplus unit,
    // one small wave at a time.  This is the opening-centre behaviour that
    // replaced the damaged B1 with B6 in the analysed replay.
    for (int target : strategy.core_targets) {
      const Building *target_building = building_at[target];
      const bool enemy_building =
          target_building != nullptr && target_building->side == enemy_side;
      if (own_base(target) || (!enemy_building && enemy_at[target] == 0))
        continue;
      const int desired = enemy_building ? std::max(5, enemy_at[target] + 3)
                                        : std::max(1, enemy_at[target] + 1);
      int already = my_at[target] + incoming[target];
      while (already < desired) {
        const Warrior *best = nullptr;
        int best_hops = INF;
        for (const auto &w : S.warriors) {
          if (w.id.side != M.my_side || w.state != WState::STATIONARY ||
              used(w.id) || is_scout_purpose(w.purpose) || !can_leave(w))
            continue;
          if (w.purpose == WPurpose::BUILD &&
              building_at[w.target] == nullptr)
            continue;
          ...
          if (hops < best_hops ||
              (hops == best_hops &&
              (best == nullptr || w.id.num < best->id.num))) {
            best = &w;
            best_hops = hops;
          }
        }
        if (best == nullptr ||
            !queue_move(*best, target,
                        enemy_building ? WPurpose::ATTACK : WPurpose::BUILD))
          break;
        ++already;
      }
    }

    auto min_level_of = [&](const std::vector<int> &regions) {
      int level = BASE_MAX_LEVEL;
      bool found = false;
      for (int region : regions) {
        const Building *b = building_at[region];
        if (b == nullptr || b->side != M.my_side || b->type != BType::BASE)
          continue;
        found = true;
        level = std::min(level, b->level);
      }
      return found ? level : 0;
    };
    auto upgrade_one_front_to_back = [&](const std::vector<int> &regions,
                                        int maximum_level) {
      int tier = maximum_level;
      for (int region : regions) {
        const Building *b = building_at[region];
        if (b != nullptr && b->side == M.my_side && b->type == BType::BASE &&
            b->level < maximum_level)
          tier = std::min(tier, b->level);
      }
      std::vector<int> candidates;
      for (int region : regions) {
        const Building *b = building_at[region];
        if (b != nullptr && b->side == M.my_side && b->type == BType::BASE &&
            b->level == tier && b->level < maximum_level)
          candidates.push_back(region);
      }
      std::sort(candidates.begin(), candidates.end(), [&](int lhs, int rhs) {
        if (P.dist[M.opp_hq][lhs] != P.dist[M.opp_hq][rhs])
      ...
      for (int region : candidates)
        if (queue_upgrade(region))
          return true;
      return false;
    };

    bool core_level_two = all_core_owned;
    for (int region : strategy.core_targets) {
      const Building *b = building_at[region];
      if (b == nullptr || b->side != M.my_side || b->type != BType::BASE ||
          b->level < 2)
        core_level_two = false;
    }

    bool core_maxed = all_core_owned;
    for (int region : strategy.core_targets) {
      const Building *b = building_at[region];
      if (b == nullptr || b->side != M.my_side) {
        if (!core_deadlock_fallback)
          core_maxed = false;
        continue;
      }
    ...
    // Once the two-corridor probe starts, stop spending its training money on
    // deeper core upgrades.  If no attack axis opens, the pause expires and
    // ordinary development resumes.
    const bool corridor_development_pause =
        strategy.corridor_probe_start_turn >= 0 &&
        turn - strategy.corridor_probe_start_turn <= 30;
    if (hq_level >= 3 && all_core_owned && !core_maxed &&
        !corridor_development_pause)
      upgrade_one_front_to_back(strategy.core_targets, BASE_MAX_LEVEL);

    // Once the map-sized core reaches level three, three simultaneous builders
    // claim the remaining home-side strongholds, just as in both source replays.
    if (core_maxed)
      while (count_pending_builds() < 3 &&
            dispatch_builder(strategy.home_targets, 3, false)) {
      }

    bool all_home_owned = true;
    int home_owned_count = 0;
    std::vector<int> extra_home_bases;
    for (int region : strategy.home_targets) {
      if (!own_base(region)) {
        all_home_owned = false;
      } else {
        ++home_owned_count;
      }
      if (!contains_region(strategy.core_targets, region) && own_base(region))
        extra_home_bases.push_back(region);
    }
    const bool home_deadlock_fallback =
        turn >= 180 && home_owned_count + 1 >= (int)strategy.home_targets.size();
    all_home_owned = all_home_owned || home_deadlock_fallback;
    bool home_level_two = all_home_owned;
    for (int region : strategy.home_targets) {
      const Building *b = building_at[region];
      if (b == nullptr || b->side != M.my_side) {
        if (!home_deadlock_fallback)
          home_level_two = false;
        continue;
      }
      if (b->level < 2)
        home_level_two = false;
    }
    if (core_maxed && !extra_home_bases.empty() && !home_level_two)
      upgrade_one_front_to_back(extra_home_bases, 2);

    // Construction/upgrades open jobs.  Fill those jobs before turning any
    // surplus into scouts or soldiers.
    std::vector<int> projected_cap(N, 0), projected_workers = stationary_regular_at;
    for (const auto &b : S.buildings)
      if (b.side == M.my_side)
        projected_cap[b.region] = b.work_cap();
    for (int region : a.upgrades) {
      const Building *b = building_at[region];
      if (b == nullptr) {
        projected_cap[region] = BASE_LEVELS[1].work_cap;
      } else if (b->side == M.my_side && b->level < max_level(*b)) {
        projected_cap[region] =
            b->type == BType::HQ ? HQ_LEVELS[b->level + 1].work_cap
                                : BASE_LEVELS[b->level + 1].work_cap;
      }
    }
    for (int region = 0; region < N; ++region)
      projected_workers[region] -= leaving[region];
    for (const auto &move : a.moves)
      if (!is_scout_purpose(move.purpose))
        ++projected_workers[move.target];
    ...
    while (true) {
      ...
      int best_hops = INF;
      for (int target = 0; target < N; ++target) {
        if (projected_workers[target] >= projected_cap[target])
          continue;
        for (const auto &w : S.warriors) {
          if (w.id.side != M.my_side || w.state != WState::STATIONARY ||
              used(w.id) || is_scout_purpose(w.purpose) || !can_leave(w) ||
              projected_workers[w.region] <= projected_cap[w.region])
            continue;
          if (w.purpose == WPurpose::BUILD &&
              building_at[w.target] == nullptr)
            continue;
      ...
      if (best == nullptr || !queue_move(*best, best_target, WPurpose::NONE))
        break;
      --projected_workers[best->region];
      ++projected_workers[best_target];
    }

    // Clean up the transient-scout registry.
    strategy.scout_started.erase(
        std::remove_if(strategy.scout_started.begin(),
                      strategy.scout_started.end(), [&](const auto &entry) {
                        const Warrior *w = nullptr;
                        for (const auto &candidate : S.warriors)
                        ...
                        return w == nullptr || !is_scout_purpose(w->purpose);
                      }),
        strategy.scout_started.end());
    auto scout_start_turn = [&](WarriorId id) {
      for (const auto &entry : strategy.scout_started)
        if (entry.first == id)
          return entry.second;
      return turn;
    };
    auto nearest_own_base = [&](int source) {
      int best = -1, best_hops = INF;
      for (const auto &b : S.buildings) {
        if (b.side != M.my_side)
          continue;
        const int hops = P.hops[source][b.region];
        if (hops < best_hops ||
            (hops == best_hops && (best == -1 || b.region < best))) {
          best = b.region;
          best_hops = hops;
        }
      }
    ...
    auto scout_safe = [&](int region) {
      if (region < 0 || region >= N || enemy_at[region] > 0)
        return false;
      const Building *b = building_at[region];
      if (b != nullptr && b->side == enemy_side)
        return false;
      for (const auto &w : S.warriors)
        if (w.id.side == enemy_side &&
            P.vision_hops[region][w.region] <= 1)
          return false;
      return true;
    };
    auto scout_leg = [&](const Warrior &scout, int objective) {
      if (objective < 0 || objective >= N)
        return -1;
      if (P.vision_hops[scout.region][objective] <= HOP_VISION)
        return -1;
      std::vector<int> previous(N, -1);
      std::queue<int> queue;
      previous[scout.region] = scout.region;
      queue.push(scout.region);
      int reached = -1;
      while (!queue.empty()) {
        const int region = queue.front();
        queue.pop();
        if (region != scout.region &&
            P.vision_hops[region][objective] <= HOP_VISION) {
          reached = region;
          break;
        }
        for (int next : M.adj[region]) {
          if (previous[next] != -1 || !scout_safe(next))
            continue;
          previous[next] = region;
          queue.push(next);
        }
    ...
    // Replay 37 sends two fresh warriors ahead of the army on different
    // corridors.  Unlock the same rolling probe as soon as the core economy is
    // useful (all core bases at least level two) and HQ level three has
    // resolved.  The objectives are enemy-side strongholds whose weighted
    // routes from the front base overlap as little as possible.
    int corridor_rally_hub = -1;
    int corridor_rally_hops = INF;
    for (int region : strategy.core_targets) {
      if (!own_base(region))
        continue;
      const int hops = P.hops[region][M.opp_hq];
      if (hops < corridor_rally_hops ||
          (hops == corridor_rally_hops &&
           (corridor_rally_hub == -1 || region < corridor_rally_hub))) {
        corridor_rally_hub = region;
        corridor_rally_hops = hops;
      }
    }
    if (corridor_rally_hub == -1)
      corridor_rally_hub = M.my_hq;

    const bool corridor_probe_unlocked = core_level_two && hq_level >= 3;
    if (corridor_probe_unlocked && strategy.corridor_probe_start_turn < 0) {
      strategy.corridor_probe_start_turn = turn;
      std::vector<int> objectives;
      for (int region : M.strongholds)
        if (region != M.opp_hq &&
            P.dist[M.opp_hq][region] < P.dist[M.my_hq][region] &&
            P.hops[corridor_rally_hub][region] < INF)
          objectives.push_back(region);
      if (objectives.empty())
        objectives.push_back(M.opp_hq);
      std::sort(objectives.begin(), objectives.end(), [&](int lhs, int rhs) {
        const int lhs_hops = P.hops[corridor_rally_hub][lhs];
        const int rhs_hops = P.hops[corridor_rally_hub][rhs];
        return lhs_hops != rhs_hops ? lhs_hops < rhs_hops : lhs < rhs;
      });

      strategy.corridor_probe_targets.push_back(objectives.front());
      if (objectives.size() > 1) {
        std::vector<char> first_route(N, false);
        int cur = corridor_rally_hub;
        for (int guard = 0; guard < N && cur != objectives.front(); ++guard) {
          cur = P.nxt[cur][objectives.front()];
          if (cur < 0)
            break;
          first_route[cur] = true;
        }
        int second = -1;
        long long best_score = std::numeric_limits<long long>::max();
        for (int candidate : objectives) {
          if (candidate == objectives.front())
            continue;
          int overlap = 0;
          cur = corridor_rally_hub;
          for (int guard = 0; guard < N && cur != candidate; ++guard) {
            cur = P.nxt[cur][candidate];
            if (cur < 0)
              break;
            overlap += first_route[cur] ? 1 : 0;
          }
          const long long score = (long long)overlap * N * 4 +
                                  P.hops[corridor_rally_hub][candidate];
          if (score < best_score ||
              (score == best_score && (second == -1 || candidate < second))) {
            best_score = score;
            second = candidate;
          }
        }
        if (second >= 0)
          strategy.corridor_probe_targets.push_back(second);
      }
      dbg::note(turn, "CORRIDOR_PROBE start hub=R" +
                          std::to_string(corridor_rally_hub) + " targets=" +
                          std::to_string(strategy.corridor_probe_targets.size()));
    }

    // Remove dead/recycled assignments, advance living scouts one safe edge at
    // a time, and return them to the rally hub immediately after observation.
    strategy.corridor_scout_targets.erase(
        std::remove_if(strategy.corridor_scout_targets.begin(),
                       strategy.corridor_scout_targets.end(),
                       [&](const auto &assignment) {
                         for (const auto &w : S.warriors)
                           if (w.id == assignment.first)
                             return w.purpose != WPurpose::OFFENSE_SCOUT;
                         return true;
                       }),
        strategy.corridor_scout_targets.end());
    for (const auto &assignment : strategy.corridor_scout_targets) {
      const Warrior *scout = nullptr;
      for (const auto &w : S.warriors)
        if (w.id == assignment.first) {
          scout = &w;
          break;
        }
      if (scout == nullptr || scout->state != WState::STATIONARY ||
          used(scout->id))
        continue;
      const int objective = assignment.second;
      const bool observed =
          P.vision_hops[scout->region][objective] <= HOP_VISION &&
          S.last_seen_region_turn[objective] >=
              strategy.corridor_probe_start_turn;
      if (observed) {
        if (scout->region != corridor_rally_hub)
          queue_move(*scout, corridor_rally_hub, WPurpose::NONE, false);
        continue;
      }
      const int leg = scout_leg(*scout, objective);
      if (leg >= 0)
        queue_move(*scout, leg, WPurpose::OFFENSE_SCOUT, false);
    }

    const bool corridor_probe_recruiting =
        strategy.corridor_probe_start_turn >= 0 &&
        turn - strategy.corridor_probe_start_turn <= 30;
    while (corridor_probe_recruiting &&
           strategy.corridor_scout_targets.size() <
               strategy.corridor_probe_targets.size()) {
      int objective = -1;
      for (int target : strategy.corridor_probe_targets) {
        bool assigned_target = false;
        for (const auto &assignment : strategy.corridor_scout_targets)
          if (assignment.second == target)
      ...
      for (const auto &w : S.warriors) {
        ...
        if (w.purpose == WPurpose::BUILD &&
            building_at[w.target] == nullptr)
          continue;
        const int leg = scout_leg(w, objective);
        if (leg < 0)
          continue;
        const int hq_source = w.region == M.my_hq ? 1 : 0;
        const int hops = P.hops[w.region][objective];
        if (best == nullptr || hq_source > best_hq_source ||
            (hq_source == best_hq_source && hops < best_hops) ||
            (hq_source == best_hq_source && hops == best_hops &&
             w.id.num > best->id.num)) {
          best = &w;
          best_hq_source = hq_source;
          best_hops = hops;
        }
      }
      if (best == nullptr)
        break;
      const int leg = scout_leg(*best, objective);
      if (leg < 0 ||
          !queue_move(*best, leg, WPurpose::OFFENSE_SCOUT, false))
        break;
      strategy.corridor_scout_targets.push_back({best->id, objective});
      dbg::note(turn, "CORRIDOR_SCOUT " + format_warrior(best->id) +
                          " objective=R" + std::to_string(objective) +
                          " leg=R" + std::to_string(leg));
    }

    if (core_maxed && hq_level >= 3) {
      // Existing scouts receive one-edge orders.  After one sweep they are
      // recycled into the closest workplace instead of becoming permanent HQ
      // sentries.
      for (const auto &w : S.warriors) {
        if (w.id.side != M.my_side || w.purpose != WPurpose::SCOUT ||
            w.state != WState::STATIONARY || used(w.id))
          continue;
        if (turn - scout_start_turn(w.id) >= SCOUT_LIFETIME) {
          const int home = nearest_own_base(w.region);
          if (home >= 0 && home != w.region)
            queue_move(w, home, WPurpose::NONE, false);
          continue;
        }
        int objective = -1, best_age = -1, best_hops = INF;
        for (int region : M.strongholds) {
          if (region == M.opp_hq ||
              P.dist[M.opp_hq][region] >= P.dist[M.my_hq][region])
            continue;
          const int last_seen = S.last_seen_region_turn[region];
          const int age = last_seen < 0 ? MAX_TURN + turn : turn - last_seen;
          const int hops = P.vision_hops[w.region][region];
          if (P.vision_hops[w.region][region] <= HOP_VISION)
            continue;
          if (age > best_age ||
              (age == best_age && hops < best_hops) ||
              (age == best_age && hops == best_hops && region < objective)) {
            objective = region;
            best_age = age;
            best_hops = hops;
          }
        }
        const int leg = objective >= 0 ? scout_leg(w, objective) : -1;
        if (leg >= 0)
          queue_move(w, leg, WPurpose::SCOUT, false);
      }
      ...
      for (const auto &w : S.warriors)
        if (w.id.side == M.my_side && is_scout_purpose(w.purpose))
          ++active_scouts;
      const int desired_scouts = 3;
      while (active_scouts < desired_scouts) {
        const Warrior *best = nullptr;
        int objective = -1, leg = -1, best_age = -1;
        for (const auto &w : S.warriors) {
          if (w.id.side != M.my_side || w.state != WState::STATIONARY ||
              used(w.id) || is_scout_purpose(w.purpose) || !can_leave(w))
            continue;
          ...
          for (int region : M.strongholds) {
            if (P.dist[M.opp_hq][region] >= P.dist[M.my_hq][region] ||
                P.vision_hops[w.region][region] <= HOP_VISION)
              continue;
            const int seen = S.last_seen_region_turn[region];
            const int age = seen < 0 ? MAX_TURN + turn : turn - seen;
            const int candidate_leg = scout_leg(w, region);
            if (candidate_leg < 0)
              continue;
            if (age > best_age ||
                (age == best_age &&
        ...
        (void)objective;
        if (best == nullptr || !queue_move(*best, leg, WPurpose::SCOUT))
          break;
        strategy.scout_started.push_back({best->id, turn});
        ++active_scouts;
      }
    }

    int owned_bases = 0, owned_strongholds = 0;
    std::vector<int> own_base_regions;
    for (const auto &b : S.buildings) {
      if (b.side == M.my_side && b.type == BType::BASE) {
        ++owned_bases;
        ++owned_strongholds;
        own_base_regions.push_back(b.region);
      }
    }

    bool corridor_attack_ready = false;
    if (strategy.corridor_probe_start_turn >= 0)
      for (const auto &b : S.buildings)
        if (b.side == enemy_side && b.type == BType::BASE &&
            b.last_seen_turn >= strategy.corridor_probe_start_turn &&
            turn - b.last_seen_turn <= 6)
          corridor_attack_ready = true;
    const bool full_attack_economy_ready = core_maxed && home_level_two;
    bool attack_economy_ready =
        full_attack_economy_ready || (core_level_two && corridor_attack_ready);
    std::vector<int> enemy_base_regions;
    for (const auto &b : S.buildings)
      if (b.side == enemy_side && b.type == BType::BASE)
        enemy_base_regions.push_back(b.region);

    // Level three is reserved for the economic core and positions that are
    // actually relevant to the front.  A remote home-side outpost still gets
    // level two for its second job, but it no longer delays HQ 4/5 merely
    // because it exists.
    auto needs_level_three = [&](int region) {
      if (contains_region(strategy.core_targets, region) ||
          P.hops[M.my_hq][region] <= 2)
        return true;
      for (const auto &b : S.buildings)
        if (b.side == enemy_side && b.type == BType::BASE &&
            turn - b.last_seen_turn <= 12 &&
            P.hops[region][b.region] <= 4)
          return true;
      for (const auto &w : S.warriors)
        if (w.id.side == enemy_side &&
            P.vision_hops[region][w.region] <= HOP_VISION)
          return true;
      return false;
    };

    auto movable_group = [&](int source) {
      std::vector<const Warrior *> group;
      for (const auto &w : S.warriors) {
        if (w.id.side != M.my_side || w.region != source ||
            w.state != WState::STATIONARY || used(w.id) ||
      ...
      const int movable = std::max(
          0, stationary_regular_at[source] - leaving[source] - keep_at[source]);
      if ((int)group.size() > movable)
        group.resize(movable);
      return group;
    };
    auto hq_assault_group = [&](int source) {
      std::vector<const Warrior *> group;
      for (const auto &w : S.warriors) {
        if (w.id.side != M.my_side || w.region != source ||
            w.state != WState::STATIONARY || used(w.id) ||
      ...
      const int last_guard =
          source_building != nullptr && source_building->side == M.my_side
              ...
              : 0;
      const int movable = std::max(
          0, stationary_regular_at[source] - leaving[source] - last_guard);
      if ((int)group.size() > movable)
        group.resize(movable);
      return group;
    };
    auto defender_hps = [&](int target) {
      std::vector<int> hps;
      for (const auto &intel : S.enemy_memory) {
        if (intel.region != target)
          continue;
        if (is_visible[target] && !intel.visible)
          continue;
        hps.push_back(std::max(0, intel.hp));
      }
      return hps;
    };
    ...
    auto choose_attack = [&]() {
      AttackChoice best;
      if (!attack_economy_ready)
        return best;
      for (const auto &target : S.buildings) {
        if (target.side != enemy_side || target.type != BType::BASE)
          continue;
        const int age = std::max(0, turn - target.last_seen_turn);
        if (age > 20)
          continue;
        const std::vector<int> defenders = defender_hps(target.region);
        for (int source = 0; source < N; ++source) {
          std::vector<const Warrior *> group = movable_group(source);
          if (group.empty())
            continue;
          std::vector<int> hps;
          for (const Warrior *w : group)
            hps.push_back(w->hp);
          int need = minimum_attackers(target, defenders, hps);
          if (need >= INF)
            continue;
          need += ATTACK_MARGIN;
          if ((int)group.size() < need)
            continue;
          const int travel = P.hops[source][target.region];
          if (travel >= INF)
            continue;
          // Weak observed garrisons dominate distance.  This reproduces the
          // replay's R61/R70 choice ahead of merely closer but reinforced R63.
          const int score = need * 2000 + (int)defenders.size() * 600 +
                            travel * 120 + target.level * 80;
          if (score < best.score ||
              (score == best.score && target.region < best.target->region))
            best = AttackChoice{&target, source, need, score, std::move(group)};
        }
      }
      return best;
    };

    if (!strategy.hq_assault_started) {
      const int wave_limit = full_attack_economy_ready ? 2 : 1;
      for (int wave = 0; wave < wave_limit; ++wave) {
        AttackChoice choice = choose_attack();
        if (choice.target == nullptr)
          break;
        int sent = 0;
        for (const Warrior *w : choice.group) {
          if (sent >= choice.count)
            break;
          if (queue_move(*w, choice.target->region, WPurpose::ATTACK))
            ++sent;
        }
        if (sent == 0)
          break;
        strategy.last_attack_turn[choice.target->region] = turn;
      }
    }

    // Newly conquered enemy-side bases are fortified before technology is
    // deepened.  Core/home-base upgrades above remain breadth-first.
    std::vector<int> conquered;
    for (int region : own_base_regions)
      if (!contains_region(strategy.home_targets, region) &&
          needs_level_three(region))
        conquered.push_back(region);
    if (attack_economy_ready && !conquered.empty())
      upgrade_one_front_to_back(conquered, BASE_MAX_LEVEL);

    // The late home bases first reach level two for their jobs.  Only an HQ-
    // adjacent or genuine frontline member is deepened to level three; remote
    // outposts must not consume the HQ technology fund.
    if (attack_economy_ready && a.upgrades.empty() &&
        !extra_home_bases.empty()) {
      std::vector<int> strategic_extra_bases;
      for (int region : extra_home_bases)
        if (needs_level_three(region))
          strategic_extra_bases.push_back(region);
      if (!strategic_extra_bases.empty())
        upgrade_one_front_to_back(strategic_extra_bases, BASE_MAX_LEVEL);
    }

    // Project today's base upgrades before testing the next technology stage.
    // HQ 4/5 require the complete home economy and every strategically relevant
    // base at level three.  Remote level-two outposts are deliberately ignored.
    bool all_priority_bases_maxed = true;
    int next_owned_base_upgrade_cost = INF;
    for (int region : own_base_regions) {
      if (!needs_level_three(region))
        continue;
      const Building *b = building_at[region];
      int projected_level = b != nullptr ? b->level : 0;
      if (b != nullptr && projected_level < BASE_MAX_LEVEL &&
          queued_upgrade(region))
        ++projected_level;
      if (projected_level < BASE_MAX_LEVEL) {
        all_priority_bases_maxed = false;
        next_owned_base_upgrade_cost = std::min(
            next_owned_base_upgrade_cost,
            BASE_LEVELS[projected_level + 1].cost);
      }
    }
    for (int region : a.upgrades) {
      if (building_at[region] != nullptr || !needs_level_three(region))
        continue;
      all_priority_bases_maxed = false;
      next_owned_base_upgrade_cost =
          std::min(next_owned_base_upgrade_cost, BASE_LEVELS[2].cost);
    }
    const bool home_development_maxed =
        all_home_owned && all_priority_bases_maxed;

    if (hq != nullptr && hq->level >= 3 && hq->level < HQ_MAX_LEVEL &&
        home_development_maxed && queue_upgrade(M.my_hq))
      dbg::note(turn, "HQ_STAGE bases_maxed level=" +
                          std::to_string(hq->level) + "->" +
                          std::to_string(hq->level + 1) + " gold=" +
                          std::to_string(S.gold) + " bases=" +
                          std::to_string(owned_bases));

    int projected_hq_level = hq_level;
    if (queued_upgrade(M.my_hq) && projected_hq_level < HQ_MAX_LEVEL)
      ++projected_hq_level;

    const bool territory_complete =
        owned_strongholds == (int)M.strongholds.size() && enemy_base_regions.empty();
    const int mass_threshold =
        HQ_LEVELS[std::max(1, projected_hq_level)].work_cap +
        std::max(80, owned_strongholds * 18);
    if (!strategy.hq_assault_started && territory_complete &&
        projected_hq_level >= HQ_MAX_LEVEL && my_alive >= mass_threshold)
      strategy.hq_assault_started = true;

    // Select the next objective for rallying.  Before the last assault it is the
    // weakest recent base; afterwards it is the enemy HQ.
    int rally_target = strategy.hq_assault_started ? M.opp_hq : -1;
    if (rally_target == -1) {
      int best_score = INF;
      for (const auto &b : S.buildings) {
        if (b.side != enemy_side || b.type != BType::BASE)
          continue;
        const int age = std::max(0, turn - b.last_seen_turn);
        const int score = age * 100 + (int)defender_hps(b.region).size() * 1000;
        if (score < best_score) {
          best_score = score;
          rally_target = b.region;
        }
      }
    }
    if (rally_target == -1 && corridor_probe_recruiting)
      rally_target = M.opp_hq;
    ...
    if (rally_target != -1) {
      ...
      for (const auto &b : S.buildings) {
        if (b.side != M.my_side || b.type != BType::BASE)
          continue;
        const int hops = P.hops[b.region][rally_target];
        if (hops < best_hops ||
            (hops == best_hops && b.region < rally_hub)) {
          rally_hub = b.region;
          best_hops = hops;
        }
      }
    ...
    // Final HQ assault: a 22-unit opening body followed by a seven-unit stream.
    if (strategy.hq_assault_started) {
      int wanted = 7;
      bool already_sent = false;
      for (const auto &move : a.moves)
        if (move.purpose == WPurpose::ATTACK && move.target == M.opp_hq)
          already_sent = true;
      if (!already_sent) {
        int currently_targeting = 0;
        for (const auto &w : S.warriors)
          if (w.id.side == M.my_side && w.state == WState::MOVING &&
              w.target == M.opp_hq)
            ++currently_targeting;
        if (currently_targeting == 0)
          wanted = 22;
        for (int source = 0; source < N && wanted > 0; ++source) {
          std::vector<const Warrior *> group = hq_assault_group(source);
          for (const Warrior *w : group) {
            if (wanted == 0)
              break;
            if (queue_move(*w, M.opp_hq, WPurpose::ATTACK, false, false))
              --wanted;
          }
        }
      }
    ...
    // Free transfers through owned bases create the replay's large forward
    // rally stacks.  Keep every workplace filled and move only genuine surplus.
    if ((attack_economy_ready || corridor_probe_recruiting) && rally_hub >= 0) {
      int rallied = 0;
      for (const auto &w : S.warriors) {
        if (rallied >= 120 || w.id.side != M.my_side ||
            w.state != WState::STATIONARY || used(w.id) ||
            is_scout_purpose(w.purpose) || w.region == rally_hub ||
            !can_leave(w))
          continue;
        if (w.purpose == WPurpose::BUILD &&
            building_at[w.target] == nullptr)
          continue;
        if (queue_move(w, rally_hub, WPurpose::NONE))
          ++rallied;
      }
    }

    // Compute the economic force after today's upgrades.  Training uses only
    // money left after the next concrete construction/upgrade commitment.
    int projected_work_cap = 0;
    for (int region = 0; region < N; ++region)
      projected_work_cap += projected_cap[region];
    int active_scouts = 0;
    for (const auto &w : S.warriors)
      if (w.id.side == M.my_side && is_scout_purpose(w.purpose))
        ++active_scouts;
    int desired_alive = projected_work_cap + count_pending_builds();
    if (corridor_probe_recruiting)
      desired_alive += active_scouts + std::max(14, owned_bases * 2);
    else if (core_maxed)
      desired_alive += active_scouts + std::max(12, owned_bases * 2);
    else
      desired_alive += 1;
    // Enemy TRAIN events are public and enemy_memory removes confirmed deaths,
    // so its size is a conservative force-parity floor.  The old economic
    // target could stop at work capacity while the opponent kept producing for
    // hundreds of turns; matching total surviving bodies prevents that silent
    // military collapse without changing the development order itself.
    // Large idle cash is converted into a late mobile reserve instead of being
    // allowed to grow into five figures.  This clause matters only after the
    // normal construction reserve has already become a small fraction of the
    // treasury.
    const bool cash_overflow = S.gold >= 2400;
    const int remembered_enemy_alive = (int)S.enemy_memory.size();
    const bool late_force_management = turn >= 220 || cash_overflow;
    if (late_force_management)
      desired_alive = std::max(desired_alive, remembered_enemy_alive + 2);
    if (cash_overflow)
      desired_alive = std::max(
          desired_alive,
          projected_work_cap + active_scouts +
              std::max(24, owned_bases * 4));
    if (territory_complete && projected_hq_level >= HQ_MAX_LEVEL)
      desired_alive = std::max(desired_alive, mass_threshold);

    int reserve = 0;
    if (corridor_probe_recruiting)
      reserve = 0;
    else if (home_development_maxed && projected_hq_level >= 3 &&
        projected_hq_level < HQ_MAX_LEVEL)
      reserve = HQ_LEVELS[projected_hq_level + 1].upgrade_cost;
    else if (count_pending_builds() > 0)
      reserve = BASE_LEVELS[1].cost;
    else if (all_core_owned && hq_level == 2)
      reserve = HQ_LEVELS[3].upgrade_cost;
    else if (hq_level >= 3 && !core_maxed)
      reserve = min_level_of(strategy.core_targets) <= 1
                    ? BASE_LEVELS[2].cost
                    : BASE_LEVELS[3].cost;
    else if (core_maxed && !all_home_owned)
      reserve = BASE_LEVELS[1].cost;
    else if (next_owned_base_upgrade_cost < INF)
      reserve = next_owned_base_upgrade_cost;

    int defense_shortage = 0;
    for (const auto &b : S.buildings)
      if (b.side == M.my_side)
        defense_shortage +=
            std::max(0, keep_at[b.region] - my_at[b.region] - incoming[b.region]);
    bool urgent_enemy_near_building = false;
    for (const auto &b : S.buildings) {
      if (b.side != M.my_side)
        continue;
      for (const auto &w : S.warriors)
        if (w.id.side == enemy_side &&
            P.vision_hops[w.region][b.region] <= 1)
          urgent_enemy_near_building = true;
    }
    const bool saving_for_hq_stage =
        (all_core_owned && hq_level == 2) ||
        (home_development_maxed && projected_hq_level >= 3 &&
         projected_hq_level < HQ_MAX_LEVEL);
    // Routine staffing gaps and two-hop sightings do not cancel a concrete
    // development reserve.  A confirmed enemy at/adjacent to an own building
    // may release base-development money for defense, while an unlocked HQ
    // stage remains a hard replay priority.
    if (defense_shortage > 0 && urgent_enemy_near_building &&
        !saving_for_hq_stage)
      reserve = 0;

    const int train_cap = HQ_LEVELS[projected_hq_level].train_cap;
    const bool continuous_max_hq_training =
        projected_hq_level >= HQ_MAX_LEVEL;
    const int want = std::max(
        {defense_shortage, desired_alive - my_alive,
         continuous_max_hq_training ? train_cap : 0});
    int regular_alive = 0;
    for (const auto &w : S.warriors)
      if (w.id.side == M.my_side && !is_scout_purpose(w.purpose))
        ++regular_alive;
    const bool severe_workforce_shortage =
        regular_alive + std::max(2, train_cap) < projected_work_cap;
    const bool force_parity_shortage =
        late_force_management && remembered_enemy_alive > my_alive + 2;
    // Do not wait forever for the next 550/600-gold upgrade while income has
    // collapsed or the opponent has a clear force lead.  One TRAIN restores a
    // worker and therefore income, allowing development to resume naturally.
    if (continuous_max_hq_training ||
        (want > 0 && (severe_workforce_shortage || force_parity_shortage ||
                      cash_overflow)))
      reserve = 0;
    if (!strategy.hq_assault_started && hq_probe_move_reserve > 0)
      reserve = std::max(reserve, hq_probe_move_reserve);
    const int affordable = std::max(0, (budget - reserve) / TRAIN_COST);
    const int additional_train = std::max(
        0, std::min({train_cap - forced_builder_train_n,
                    std::max(0, want - forced_builder_train_n), affordable}));
    a.train_n = forced_builder_train_n + additional_train;
    budget -= additional_train * TRAIN_COST;

    dbg::note(turn, "IMITATION phase core=" +
                        std::string(core_maxed ? "MAX" :
                                    (all_core_owned ? "TECH" : "EXPAND")) +
                        " home2=" + (home_level_two ? "1" : "0") +
                        " bases=" + std::to_string(owned_bases) +
                        " scouts=" + std::to_string(active_scouts) +
                        " corridor=" +
                        (corridor_probe_recruiting ? "1" : "0") +
                        " corridor_attack=" +
                        (corridor_attack_ready ? "1" : "0") +
                        " hq_assault=" +
                        (strategy.hq_assault_started ? "1" : "0") +
                        " enemy_alive=" +
                        std::to_string(remembered_enemy_alive) +
                        " max_hq_train=" +
                        (continuous_max_hq_training ? "1" : "0") +
                        " train=" + std::to_string(a.train_n) +
  ...
  int main() {
    ...
    while (read_turn_start(turn)) {
      dbg::turn_header(turn, S, M);
      Actions actions = decide_sample_imitation(S, M, P, strategy, turn);
      emit_command();
      emit_actions(actions);
      emit_end();
      read_turn_result(S, M, actions);
    }
    return 0;
  }
