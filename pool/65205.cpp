...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  // 사령부가 지금 당장 레벨업 가능한 금액에 못 미치면, 기지가 지금 당장
  // 업그레이드 가능하더라도 그 예산을 미리 써버리지 않고 저축해서 사령부
  // 레벨업을 앞당길지 결정한다. 저축 비율은 턴이 진행될수록(lateness) 커져서,
  // 게임 후반으로 갈수록 "당장 되는 기지 업그레이드"보다 "사령부를 위한 저축"을
  // 더 우선시하게 된다. 사령부가 이미 당장 감당되면(upgrade_budget >= hq_cost)
  // 아래 루프에서 사령부부터 바로 사버리므로 이 저축은 필요 없다.
  // (이미 이번 턴에 사령부를 올렸다면 저축할 필요가 없으므로 건너뛴다)
