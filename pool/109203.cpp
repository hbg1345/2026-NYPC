...
// 내 수비 병력이 전멸하지 않고 살아남는(그리고 공격측이 먼저 전멸하는)
// 데 필요한 최소 수비 인원을 구한다. 건물이 버티는 것만으로는 부족하다 —
// 포탑 화력에만 기대 건물은 살아남아도 내 병력이 다 죽으면 그 거점이 텅
// 비어, 상주 병력이 사라진 채로 다음 물결에 그대로 뚫리기 때문이다.
...
static int min_regional_defenders(int bldHp, int turret,
  ...
  auto simulate = [&](int defenders) {
    ...
    // 공격측이 전멸하거나 내 수비 병력이 전멸할 때까지 싸운다.
    while (!aF.empty() && !defF.empty()) {
      combatDay(defF, turret, hp, aF, 0, aBldHpDummy);
    }
    // 공격측을 전멸시키고도 내 병력이 남아 있어야 방어 성공(건물만
    // 버티고 내 병력이 다 죽는 경우는 실패로 본다).
    return aF.empty() && !defF.empty();
  };
...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  for (const auto &w : S.warriors) {
    ...
    for (int t : M.strongholds) {
      ...
      oppArrivalEta[t] = std::min(oppArrivalEta[t], hops(w.region, t));
  ...
  // === 출정 중(MOVING+BUILD) 병력의 목표 재검증/재조준 ===
  // 빈 거점으로 이동 중인 병력은 매 턴, 지금 향하는 목표가 아직 유효한지 다시
  // 본다. 상대가 나보다 먼저 닿는 게 확실하거나(oppArrivalEta<=내hop), 상대
  // 병력이 더 많아 지어도 못 지키거나(defensibleAfterBuild 실패), 이미
  // 지어졌거나 상대가 점거했으면 -> 지금 위치에서 갈 수 있는 다른 빈 거점 중
  // (수익성/상대미선점/방어가능) 필터를 통과하는 가장 가까운 곳으로 목표를
  // 바꿔 재조준한다. 갈 만한 곳이 없으면 목표를 유지한 채 그대로 둔다(곧
  // 도착해 idle로 풀리면 다음 턴 일반 로직이 재판단).
  for (const auto &w : S.warriors) {
    if (w.id.side != M.my_side) continue;
    if (w.state != WState::MOVING || w.purpose != WPurpose::BUILD) continue;
    int t = w.target;
    int curH = hops(w.region, t);
    bool targetGood =
        bld[t] == nullptr && oppCnt[t] == 0 &&
        oppArrivalEta[t] > curH &&
        worth_building_base(turn, curH) &&
        defensibleAfterBuild(t, available_gold, projIncome);
    if (targetGood) continue;

    // 재조준: 현재 위치에서 갈 수 있는 유효한 빈 거점 중 가장 가까운 곳.
    int newT = -1, newH = std::numeric_limits<int>::max();
    double newD = std::numeric_limits<double>::infinity();
    for (int c : M.strongholds) {
      if (c == t) continue;
      if (bld[c] != nullptr || oppCnt[c] > 0) continue;
      if (homeCnt[c] + incoming[c] >= oppCnt[c] + 1) continue; // 이미 충분히 배정됨
      int h = hops(w.region, c);
      if (h >= 9999) continue;
      if (oppArrivalEta[c] <= h) continue;                     // 상대가 먼저 닿음
      if (!worth_building_base(turn, h)) continue;             // 수익성 미달
      if (!defensibleAfterBuild(c, available_gold, projIncome)) continue; // 방어 불가
      if (h > newH) continue;
      if (h == newH && P.dist[w.region][c] >= newD) continue;
      newH = h; newD = P.dist[w.region][c]; newT = c;
    }

    if (newT == -1) continue; // 갈 만한 새 목표 없음 -> 기존 목표 유지

    // 재조준 확정: 기존 목표 커밋을 떼고 새 목표로 MOVE 재발행(빈 거점이라 MOVE_COST).
    --incoming[t];
    a.moves.push_back({w.id, newT, WPurpose::BUILD});
    gold -= MOVE_COST;
  ...
  // 막아 두었다).
  a.train_n = hqBehind ? 0 : std::max({0, nNormal, nEmergency});
