...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  int projIncome = current_net_income;
  ...
  while (!strongholdDone) {
    ...
    if (hqIdleCnt > need[hqRegion]) {
      for (int t : needing) {
        ...
        int totalCost = BASE_LEVELS[1].cost + MOVE_COST;
        int shortfall = totalCost - projIncome * h - available_gold;
        int myTurn = h;
        if (shortfall > 0) {
          myTurn = (projIncome > 0)
              ? h + (shortfall + projIncome - 1) / projIncome
        ...
        if (myTurn == bestTurn && frontD == bestFrontD &&
            P.dist[hqRegion][t] >= bestD) continue;
        bestTurn = myTurn; bestFrontD = frontD; bestD = P.dist[hqRegion][t];
        bestT = t; bestH = h;
      }
    }
