...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  // 내 사령부-상대 사령부를 잇는 선분의 수직이등분선(전선)에 어떤 거점이
  // 얼마나 가까운지를 잰다. 이 수직이등분선에 가까운 거점일수록 "누가
  // 먼저 먹느냐/누구를 노리느냐"의 경합이 실제로 팽팽한 최전선이고, 한쪽
  // 사령부 쪽으로 치우친 거점은 hops로는 비슷해 보여도 사실상 그쪽 진영
  // 소유나 다름없어 긴급성이 낮다. 두 사령부의 중점을 지나면서 두
  // 사령부를 잇는 축에 수직인 직선까지의 거리는, 그 거점 좌표를 두
  // 사령부를 잇는 축 위에 투영한 뒤 중점으로부터의 거리로 계산된다.
  double hqAxisX = (double)(M.x[M.opp_hq] - M.x[M.my_hq]);
  double hqAxisY = (double)(M.y[M.opp_hq] - M.y[M.my_hq]);
  double hqAxisLen = std::sqrt(hqAxisX * hqAxisX + hqAxisY * hqAxisY);
  double hqMidX = ((double)M.x[M.my_hq] + (double)M.x[M.opp_hq]) / 2.0;
  double hqMidY = ((double)M.y[M.my_hq] + (double)M.y[M.opp_hq]) / 2.0;
  auto frontlineDist = [&](int r) -> double {
    if (hqAxisLen < 1e-9) return 0.0;
    double qx = (double)M.x[r] - hqMidX, qy = (double)M.y[r] - hqMidY;
    double proj = (qx * hqAxisX + qy * hqAxisY) / hqAxisLen;
    return std::fabs(proj);
  };
  ...
  for (const auto &b : S.buildings) {
    ...
    for (const auto &mb : S.buildings) {
      ...
      if (frontD >= mirrorFrontD) continue;
    ...
    threat_hps[mirrorTarget].insert(threat_hps[mirrorTarget].end(),
                                    excessHps.begin(), excessHps.end());
  }
  ...
  for (int r = 0; r < N; ++r) {
    if (bld[r] != nullptr && bld[r]->side == M.my_side) {
      ...
      int defense_need = attackerHps.empty() ? 0
  ...
  while (!strongholdDone) {
    ...
    // 1) 지금 실제로 남는 병력이 있는 건물 중, "내가 실제로 지을 수 있는
    //    턴"이 가장 빠른 조합을 고른다. 단순 이동 hops(h)만 보면 이동은
    //    빨라도 건설비를 모을 때까지 계속 기다려야 하는 조합을 놓칠 수
    //    있다. 그래서 도착(h턴) 시점까지 쌓일 순수입으로도 건설비가
    //    모자라면, 그 부족분을 순수입으로 채우는 데 드는 추가 턴까지
    //    더해 "실제 완공 턴"을 추정한다. 같은 턴에 지을 수 있다면(동률),
    //    두 사령부를 잇는 축의 수직이등분선(전선)에 더 가까운, 경합이
    //    더 급한 거점을 우선한다.
    int bestT = -1, bestSrc = -1;
    int bestH = std::numeric_limits<int>::max();
    int bestTurn = std::numeric_limits<int>::max();
    double bestFrontD = std::numeric_limits<double>::infinity();
    ...
    for (int t : needing) {
      ...
      for (const auto &b : S.buildings) {
        ...
        int idleCnt = 0;
        for (const Warrior *w : idle)
          if (w->region == b.region) ++idleCnt;
        if (idleCnt <= need[b.region]) continue; // 남는 병력 없음
        int h = hops(b.region, t);
        if (!worth_building_base(turn, h)) continue;

        int totalCost = BASE_LEVELS[1].cost + MOVE_COST;
        int shortfall = totalCost - current_net_income * h - available_gold;
        int myTurn = h;
        if (shortfall > 0) {
          myTurn = (current_net_income > 0)
              ? h + (shortfall + current_net_income - 1) / current_net_income
              : std::numeric_limits<int>::max() / 2; // 수입으로는 영영 못 모음
        }

        if (myTurn > bestTurn) continue;
        if (myTurn == bestTurn && frontD > bestFrontD) continue;
        if (myTurn == bestTurn && frontD == bestFrontD &&
            P.dist[b.region][t] >= bestD) continue;
        bestTurn = myTurn; bestFrontD = frontD; bestD = P.dist[b.region][t];
        bestT = t; bestSrc = b.region; bestH = h;
      }
    }

    if (bestT != -1) {
      int base_cost = BASE_LEVELS[1].cost + MOVE_COST - (current_net_income * bestH);
      ...
      for (auto cand = idle.begin(); cand != idle.end(); ++cand) {
        if ((*cand)->region != bestSrc) continue;
      ...
      // 중복 배정되는 것을 막는다.
      int future_cost = std::max(0, BASE_LEVELS[1].cost - (current_net_income * bestH));
    ...
    // 잡아둔다. 본부에서 가장 가까운, 아직 부족한 거점을 기준으로 삼는다.
    int hqBestT = -1, hqBestH = std::numeric_limits<int>::max();
    for (int t : needing) {
      int h = hops(M.my_hq, t);
      if (!worth_building_base(turn, h)) continue;
      if (h < hqBestH) { hqBestH = h; hqBestT = t; }
    }
  ...
  if (canOffensive && !underThreat) {
    ...
    if (dominant) {
      ...
      // 빈 거점 확보(stronghold-first)와 동일한 기준: 그냥 내 사령부에서
      // hop이 가장 가까운 상대 거점을 고른다. 턴 진행도에 따라 우선순위가
      // 바뀌는 가중치(예전 lineWeight/hq_line_detour_hops)는 더 이상 쓰지
      // 않는다 — 단순히 가까운 곳부터 처리하는 게 예측하기 쉽고, 거점
      // 확보 로직과도 일관된다.
      int bestH = std::numeric_limits<int>::max();
      double bestD = std::numeric_limits<double>::infinity();
      for (const auto &b : S.buildings) {
  ...
  if (total_offensive) {
    ...
    int trainWant = std::max({(!hqBehind && extraToTrain > 0) ? extraToTrain : 0,
                              gold_lead_military, strongholdTrainNeed});
