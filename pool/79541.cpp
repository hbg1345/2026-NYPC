...
// 해당 거점을 거쳐 내 사령부에서 상대 사령부로 갈 때, 곧장 가는 것보다
// hop이 몇 개나 더 드는지(진격로 "이탈 정도"를 hop 단위로 잰 값). 직행
// 경로 위에 있는 거점은 0이고, 벗어날수록 커진다. 이동 규칙상 간선 1개는
// 항상 1턴이라(README 이동 규칙 참고) 유클리드 거리는 턴 수와 무관하므로,
// hop 수만으로 "얼마나 돌아가는가"를 그대로 잰다.
...
static int hq_line_detour_hops(const GameMap &M, const Paths &P, int region) {
  int direct = P.hops[M.my_hq][M.opp_hq];
  int via = P.hops[M.my_hq][region] + P.hops[region][M.opp_hq];
  return std::max(0, via - direct);
}
...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  // 방어(빈 거점 확보 + 인력 재배치)를 공격 여부 판단보다 먼저 결정한다.
  // 예전에는 이 로직 전체가 "공격 안 하기로 한 턴"에만 실행돼서, 총공세가
  // 켜지면 그 턴엔 방어/확장 재배치가 통째로 안 돌아갔다(공격이냐 방어냐
  // 양자택일). 이제는 순서를 바꿔 먼저 방어/확장에 필요한 인원을 실제로
  // 배치하고 idle에서 빼둔 다음, 그러고도 남는 순수 잉여만 공격 후보로
  // 넘긴다 — 그러면 방어와 공격이 같은 턴에 같이 일어날 수 있다.
  ...
  // 가까운 워리어보다 먼저 그 거점을 채가는 일이 줄어든다.
  ...
  while (!strongholdDone) {
    ...
    // 1) 지금 실제로 남는 병력이 있는 건물 중, (거점, 건물) 조합의 hop이
    //    가장 작은 것을 고른다.
    ...
    for (int t : needing) {
      for (const auto &b : S.buildings) {
        ...
        if (h > bestH) continue;
        if (h == bestH && P.dist[b.region][t] >= bestD) continue;
        bestH = h; bestD = P.dist[b.region][t]; bestT = t; bestSrc = b.region;
      }
    }
    ...
    if (bestT != -1) {
      ...
      auto it = std::find_if(idle.begin(), idle.end(), [&](const Warrior *w) {
        return w->region == bestSrc;
      });
      const Warrior *w = *it;
  ...
  for (const Warrior *w : idle) {
    ...
    // 빈 거점 확보(best_strong)는 이제 위 stronghold-first 패스에서 이미
    // 다 처리했으므로, 여기 남는 idle 병력의 재배치는 기존 아군 건물
  ...
  if (canOffensive && !underThreat) {
    ...
    if (dominant) {
      ...
      for (const auto &b : S.buildings) {
        ...
        if (d == 9999) continue;
        double score = d + lineWeight * hq_line_detour_hops(M, P, b.region);
    ...
    if (currentTarget != -1) {
      ...
      // 지금 내가 보유한 전체 병력의 1/3만큼을 여유로 잡는다.
  ...
  // (hqMaxed는 위에서 total_offensive 판단 전에 이미 계산해 둠)
  if (hqMaxed) {
  ...
  // gold_lead_military는 total_offensive 판단 전에 이미 계산해 둠(위 참고).
